...
class LogStash::Filters::Throttle < LogStash::Filters::Base
  ...
  # Setting this to -1 will cause counters to be removed after reaching the "after_count"
  # instead of after a specific time period.
  ...
  def filter(event)
    ...
    if ((@period != -1) && (@next_expiration.nil? || now >= @next_expiration)) then
    ...
    # Remove counter if "after_period" is -1 and "after_count" for this event has been reached
    if (@period == -1 && counter[:count] > @after_count) then
